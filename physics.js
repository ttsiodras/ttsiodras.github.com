...
function setupCanvasAndStartSimulation() {
    ...
    if (!is1Dsim) {
        g_canvas.removeEventListener('click', rainDrop);
        g_canvas.addEventListener('click', rainDrop);
    }
...
function rainDrop(evt) {
    ...
    yOldest[g_2d_offsets[Math.floor(y)]+Math.floor(x)] = 262144;
}
...
function simulate2d() {
    ...
    for(var i=1; i<g_height-1; i++) {
        ...
        for(var j=1; j<g_width-1; j++) {
            ...
            yOldest[lineOffset + j] = (0.999*val);
            // You do realize this is a hack, right?
            // Never ask how I ended up with these constants :-)
            var color = (127.0+224.0*0.999*val/8192);
